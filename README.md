...
⚠️ **Проблема с базой данных на Railway?** См. `QUICK_FIX_RAILWAY.md`
...
### ❌ Ошибка "no such table" на Railway/Production
...
**Быстрое решение:**
1. Добавьте переменную окружения в Railway: `DB_SYNC=true`
2. Дождитесь redeploy
3. ✅ Готово!

📖 **Подробное решение:** См. `QUICK_FIX_RAILWAY.md`
...
3. В development CORS отключен по умолчанию

### ❌ Ошибка "JWT must be provided"

**Проблема:**
```
Unauthorized: JWT must be provided
```

**Решение:**
1. Убедитесь что отправляете токен в header:
   ```
   Authorization: Bearer <ваш-токен>
   ```
2. Проверьте что токен не истек (по умолчанию 7 дней)
3. Проверьте что `JWT_SECRET` одинаковый между деплоями

### ❌ База данных пустая после redeploy

**Проблема:** Все данные исчезли после перезапуска

**Решение:**
1. Убедитесь что Persistent Volume настроен
2. Проверьте что `DB_FILE=/app/data/printcore.db` (не просто `printcore.db`)
3. Проверьте наличие volume в Railway/Render Dashboard
